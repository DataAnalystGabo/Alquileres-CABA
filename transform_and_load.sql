...
-- registros que se encuentran duplicado:
...
-- Analizando y normalizando la columna "direccion". En esta sección extraeremos el barrio en donde se ubicacan
-- los departamentos. No nos interesa la calle y altura, nos importa más adelante en el proceso de EDA identificar
-- la distribución de la oferta por barrios dentro de la Ciudad Autónoma de Buenos Aires:
...
-----------------------------------------------------------------------------------------------
-----------------------------------------------------------------------------------------------
-- Revisando la columna "tipo" me he dado cuenta que existían dos registros con la categoría "Departamento en venta"
-- y como este tipo de publicaciones no nos interesa los he eliminado de nuestro dataset:
...
-- al momento de scrappear el sitio web para aquellos campos en donde no existía un valor que pudiera ser scrapeado.Lo que logramos con 
...
-- Normalizemos los valores de las categorias "m2_totales", "m2_cubiertos" y "m2_balcon". Vamos a eliminar la unidad de medida "m2" y a
...
-- por lo que no será necesario realizar ningún operación con la función split sobre
...
-- Luego, haremos la conversión de los precios en dólares a pesos según el tipo de cambio vigente al momento del scrapping (4/02/2024 - dálar blue $1140):
ALTER TABLE alquileres.publicaciones
